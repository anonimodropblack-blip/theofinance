...
import { QuickRegisterProvider, useQuickRegister } from '@/components/QuickRegisterContext'
  ...
  return (
    <div className="min-h-screen bg-[var(--bg)] text-[var(--text)] flex">
      ...
      <button
        ...
        className="fixed bottom-5 right-5 z-40 h-14 w-14 rounded-full bg-[var(--primary)] hover:bg-[var(--primary-hover)] text-white shadow-lg shadow-[var(--primary)]/30 inline-flex items-center justify-center transition-colors"
      >
        <Plus className="h-6 w-6" strokeWidth={2.5} />
      </button>

      <QuickRegister open={qr.open} onClose={qr.closeRegister} onSaved={qr.notifySaved} />
    </div>
  )
}
